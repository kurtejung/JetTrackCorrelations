...
int main(int argc, char *argv[])
{
  ...
  int nstep_residual = 2; 
  double Pf_pt_cut = 2;

  if(dataset_type_code==e_Data_pp ||dataset_type_code > 11){do_PbPb = 0;   do_pp_tracking = 1;}
  ...
  if(is_data&&!do_PbPb){
    in_file_name = "root://xrootd.cmsaf.mit.edu//store/user/dgulhan/PPJet80_ForestTag_PYTHIA_localdb_ppJEC_merged4/PPHighPtData_ForestTag_PYTHIA_localdb_ppJEC_merged_forest_0.root";
    
  }else if(dataset_type_code > 11){
    in_file_name = "root://xrootd.cmsaf.mit.edu//store/user/dgulhan/Prod_PYTHIA_localdb_ppJEC_merged/HiForest_Private_PYTHIA_localdb_ppJEC_pthat";
    in_file_name+=dataset_pthats[dataset_type_code];
    in_file_name+= "_merged_forest_0.root";
  }else{
  ...
  TString output_file_base;
    ...
    if(is_data && !do_PbPb){
      output_file_base= "/afs/cern.ch/work/h/htrauger/pp_skim_5_14/";
    }else if(dataset_type_code > 11){
      output_file_base= "/afs/cern.ch/work/h/htrauger/OfficialPythia_5_13/";
 ...
 mixing_tree->Branch("pPAcollisionEventSelectionPA", &pPAcollisionEventSelectionPA, "pPAcollisionEventSelectionPA/I");
  ...
  for(int evi = ev_min; evi < ev_max; evi++) {
	...
	residual_corrected_pt=FF_JEC->get_residual_corrected_pt(corrected_pt,hiBin);
      ...
      if( my_ct->jtpt[j4i] < 50 && residual_corrected_pt < 50) continue;
