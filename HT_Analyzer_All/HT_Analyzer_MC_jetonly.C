...
int main(int argc, char *argv[]){
  ...
  TF1 *fit_cen = new TF1("fit_cen","[0]+[1]*x+[2]*TMath::Exp([3]*x)",0.,200.);
  ...
  if(!is_data) {
    ...
    TString vz_canvas_name = "TestVzFit_"; vz_canvas_name+= data_mc_type_strs[data_mc_type_code]; vz_canvas_name+= "_part"; vz_canvas_name+=parti; vz_canvas_name+=".png";

    vz_canvas->SaveAs(vz_canvas_name);
   
    hWeight_cent->Fit("fit_cen");
    ...
    */
	  ...
	  if(is_inclusive||j4i==highest_idx||j4i==second_highest_idx){  //only if we've got a trigger according to some criteria  AND we're not pp
		...
		for(int tracks =0; tracks < (int) me_tree->trkPt->size(); tracks++){
		  ...
		  if(j4i==highest_idx){
		    ...
		    my_hists[data_mc_type_code]->hJetTrackMELeading[ibin][ibin2][ibin3]->Fill(deta,dphi, trkweight*wvz*wcen);
		  ...
		  if(j4i==second_highest_idx){
		    ...
		    my_hists[data_mc_type_code]->hJetTrackMESubLeading[ibin][ibin2][ibin3]->Fill(deta,dphi, trkweight*wvz*wcen);
	...
	while(mevi< meptrig/2. && startovercheck < 2){  // meptrig/2 because here we're going through per event, not per jet.  We roughly assume dijets, but will normalize to shape only anyway. If we've started over once already, going through a 2nd time won't do any good.
	  ...
	  for (int ibin=0;ibin<nCBins; ibin ++){
		...
		my_hists[data_mc_type_code]->ME_TrkPt_weighted[ibin][ibin2][ibin3]->Fill(me_tree->pt->at(tracks),trkweight*wvz*wcen);
		my_hists[data_mc_type_code]->ME_TrkEta_weighted[ibin][ibin2][ibin3]->Fill(me_tree->eta->at(tracks),trkweight*wvz*wcen);
		my_hists[data_mc_type_code]->ME_TrkPhi_weighted[ibin][ibin2][ibin3]->Fill(me_tree->phi->at(tracks),trkweight*wvz*wcen);
