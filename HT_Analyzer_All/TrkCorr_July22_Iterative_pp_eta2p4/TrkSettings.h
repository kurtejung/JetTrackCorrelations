...
class TrkSettings {
  ...
  //TrkSettings that should be the same across all processes
   ...
   TrkSettings(std::string = "TrkCorrInputFile.txt", bool debug=false);
};

TrkSettings::TrkSettings(std::string inputFile, bool debug)
{
  if(debug) std::cout << "Getting settings from file" << inputFile << std::endl;
  std::ifstream f;
  ...
  if (f.is_open())
  {
    ...
    f >> doSplit;   
    f.close();
    if(debug) std::cout << "TrkSettings Loaded" << std::endl;
