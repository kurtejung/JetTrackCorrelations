...
TString dataset_type_strs[e_n_dataset_types] = {"Data2015_PbPb","Data2015_pp","HydJet15","HydJet30","HydJet50","HydJet80", "HydJet120", "HydJet170","HydJet220","HydJet280","HydJet370","Pythia15","Pythia30","Pythia50","Pythia80", "Pythia120", "Pythia170","Pythia220","Pythia280","Pythia370"};
//Hydjet80 = 5
//Pythia80 = 14

std::string dataset_type_file_names[e_n_dataset_types] = {"PbPb_5TeV_skims.txt","Data2015_pp_looseMerge.txt","PbPb_MC_Pythia6Hydjet_5TeV.txt","Hydjet30.txt","Hydjet50.txt","Hydjet80.txt", "Hydjet120.txt", "Hydjet170.txt","Hydjet220.txt","Hydjet280.txt","Hydjet370.txt","Pythia15.txt","Pythia30.txt","Pythia50.txt","Pythia80.txt", "Pythia120.txt", "Pythia170.txt","Pythia220.txt","Pythia280.txt","Pythia370.txt"};
...
bool passGenTrackCuts(float trkPt, float trkEta, int chg){
    ...
    if(chg->at(tracks)==0) return false;
...
void HT_Analyzer_All_JFFCorr2_PbPb(bool doCrab = 0, int jobID=0, int datasetTypeCode = 2, int startfile=0, int nFiles = 1){
	...
	else ReadFileList( file_names, dataset_type_file_names[dataset_type_code], true);
	...
	if(is_data){
		...
		for(int mc_type_i = 0; mc_type_i < 20; mc_type_i++){

			cout<<data_mc_type_strs[mc_type_i+1]<<endl;

			my_hists[mc_type_i+1] = new hist_class(data_mc_type_strs[mc_type_i+1], is_data);
		}
	...
	me_tree->SetBranchAddress("calo_jtpt", &me_jtpt);
        ...
        if(!is_data){
            me_tree->SetBranchAddress("pt", &me_pt);
            me_tree->SetBranchAddress("eta", &me_eta);
            me_tree->SetBranchAddress("phi", &me_phi);
			...
			for (int ibin=0;ibin<nCBins; ibin ++){
				...
				for(int j4i = 0; j4i < (int) corrpt->size(); j4i++) {
					...
					for(int tracks =0; tracks < (int) trkPt->size(); tracks++){
						...
						if(is_inclusive && j4i!=highest_idx){
							...
							if(foundBjet){
								my_hists[data_mc_type_code]->hbJetTrackSignalBackgroundNonLeading[ibin][ibin2][ibin3]->Fill(deta,dphi, trkweight*wvz*wcen);
								my_hists[data_mc_type_code]->hbJetTrackSignalBackgroundNonLeading_notrkcorr[ibin][ibin2][ibin3]->Fill(deta,dphi, wvz*wcen);
							}
						}
					...
					if(!is_data){
						...
						if(is_inclusive == kTRUE){
							...
							for(int tracks =0; tracks < (int) gpt->size(); tracks++){
								...
								if(gpt->at(tracks)<=trkPtCut) continue;
								if(chg->at(tracks)==0) continue;
								//if(!is_pp && sube->at(tracks)!=0) continue;  //only pythi for these closures
						...
						if(is_inclusive||j4i==highest_idx||j4i==second_highest_idx){  //only if we've got a trigger according to some criteria  AND we're not pp
							...
							while(dummy < meptrig){
								...
								for(int tracks =0; tracks < (int) me_trkPt->size(); tracks++){
									...
									if(is_data) data_mc_type_code=0;
                                                                        else data_mc_type_code=1;
				...
				for(int j4i = 0; j4i < (int) genpt->size(); j4i++) {
					...
					for(int tracks =0; tracks < (int) gpt->size(); tracks++){
						...
						if(gpt->at(tracks)<=trkPtCut) continue;
						if(chg->at(tracks)==0) continue;
						//if(!is_pp && sube->at(tracks)!=0) continue;
				...
				//-----------------------------------------------------------------
                                ...
                                if(!do_mixing){continue; }  //Only MC mix if you have a jet and youre looking at MC.
                                ...
                                if(!is_data){
                                    ...
                                    //dump the vz contents from the surrounding bins into a single list to prevent binning artifacts
                                    vector<int> evt_mix_list;
                                    for(int jfill=-1; jfill<2; jfill++){
                                        if(((int)jet_vzbin+jfill < 0 ) || (jet_vzbin+jfill >= vzbins->GetNbinsX())) continue;
                                        for(int ifill=0; ifill<mixing_lists.at(jet_vzbin+jfill).at(jet_cent).size(); ifill++){
                                            evt_mix_list.push_back(mixing_lists.at(jet_vzbin+jfill).at(jet_cent).at(ifill));
                                        }
                                    }

                                    unsigned int pct = gRandom->Rndm()*evt_mix_list.size(); //gRandom->Rndm()*mixing_lists.at(jet_vzbin).at(jet_cent).size();

                                    //cout << "pct: "<< pct << " of " << evt_mix_list.size() << endl;

                                    unsigned int mevi = pct;
                                    unsigned int dummy=0;
                                    while(dummy < meptrig){
                                        if((int)evt_mix_list.size()<2){
                                            cout << "Warning! vzbin: "<< jet_vzbin-1 << ", cent bin: "<< jet_cent << " is devoid of mixing events!" << endl;
                                            break;
                                        }
                                        //if(mevi>mixing_lists.at(jet_vzbin).at(jet_cent).size()){ mevi=1; }
                                        if(mevi>evt_mix_list.size()){ mevi=1; }

                                        if(mevi==0) mevi++;


                                        //if(mixing_lists.at(jet_vzbin).at(jet_cent).at(mevi-1) == evi){ 
                                        if(evt_mix_list.at(mevi-1) == evi){
                                            mevi++; continue; 
                                        }

                                        //me_tree->GetEntry(mixing_lists.at(jet_vzbin).at(jet_cent).at(mevi-1));
                                        me_tree->GetEntry(evt_mix_list.at(mevi-1));
                                        if(abs(me_vz - vz) > 0.5){ mevi++; continue; }
                                        ...
                                        //cout << "vzbin : "<< jet_vzbin << " me vzbin: "<< me_vzbin << endl;
                                        //make sure i'm doing this thing correctly...
                                        assert(TMath::Abs((int)me_vzbin-(int)jet_vzbin)<2);
                                        assert(me_cent==jet_cent);
                                        ...
                                        data_mc_type_code = 4;
                                        float me_pthatweight = 1.;
                                        if(!is_data){
                                            int ibin=0;
                                            while(me_pthat>pthatbins[ibin+1]) ibin++;
                                            me_pthatweight = (xsecs[ibin]-xsecs[ibin+1])/pthatEntries[ibin];
                                        }

                                        me_pthatweight*=fit_vz->Eval(me_vz);
                                        for(int j4i = 0; j4i < (int) genpt->size(); j4i++){
                                            ...
                                            for(int tracks =0; tracks < (int) me_trkPt->size(); tracks++){
                                                ...
                                                if(is_inclusive){
                                                    ...
                                                    deta = geneta->at(j4i) - me_trkEta->at(tracks);
                                                    dphi = genphi->at(j4i) - me_trkPhi->at(tracks);
                                                ...
                                                if(j4i==highest_idx){
                                                    deta = geneta->at(highest_idx) - me_trkEta->at(tracks);
                                                    dphi = genphi->at(highest_idx) - me_trkPhi->at(tracks);
                                                    while(dphi>(1.5*TMath::Pi())){dphi+= -2*TMath::Pi();}
                                                    while(dphi< (-0.5*TMath::Pi())){dphi+= 2*TMath::Pi();}
                                                ...
                                                if(j4i==second_highest_idx){
                                                    deta = geneta->at(second_highest_idx) - me_trkEta->at(tracks);
                                                    dphi = genphi->at(second_highest_idx) - me_trkPhi->at(tracks);
                                                    while(dphi>(1.5*TMath::Pi())){dphi+= -2*TMath::Pi();}
                                                    while(dphi<(-0.5*TMath::Pi())){dphi+= 2*TMath::Pi();}
                                        ...
                                        for(int j4i = 0; j4i < (int) corrpt->size(); j4i++){
                                            bool foundBjet = false;
                                            if(discr_csvV1->at(j4i)> 0.9) foundBjet = true;
                                        ...
                                        /*    
                                            ...
                                            // start reco jet / reco track loop
                                            for(int tracks =0; tracks < (int) me_trkPt->size(); tracks++){
                                                if(!passTrackCuts(me_trkPt->at(tracks), me_trkEta->at(tracks), me_highPurity->at(tracks), me_trkChi2->at(tracks), me_trkNdof->at(tracks), me_trkNlayer->at(tracks), me_trkNHit->at(tracks), me_pfHcal->at(tracks), me_pfEcal->at(tracks))) continue;
...
*/
        ...
        if(is_data){
            ...
            for(int mc_type_i = 0; mc_type_i < n_data_mc_types_used; mc_type_i++){
                cout<<data_mc_type_strs[mc_type_i+1]<<endl;
                cout<<mc_type_i+1<<endl;
                my_hists[mc_type_i+1]->Write(mc_type_i+1);
            }
        }
        out_file->Close();
