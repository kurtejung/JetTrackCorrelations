...
int main(int argc, char *argv[]){
  ...
  TFile *f_ref_pbpb_spectra = new TFile("./PbPb_JetSpectra.root","READ");
  TFile *f_ref_pp_spectra = new TFile("./pp_JetSpectra.root","READ");
  ...
  for(int ibin = 0; ibin<4; ibin++){
    ...
    pbpb_spectrum_lead[ibin] = (TH1D*)f_ref_pbpb_spectra->Get((TString)("PbPb_all_jets_corrpT_"+CBin_strs[ibin]+"_"+CBin_strs[ibin+1]+"_Pt100_Pt300"));
    pbpb_spectrum_lead[ibin]->SetName((TString)("PbPb_jet_specrum_lead_"+CBin_strs[ibin]+"_"+CBin_strs[ibin+1]));

    pbpb_spectrum_sub[ibin] = (TH1D*)f_ref_pbpb_spectra->Get((TString)("PbPb_all_jets_corrpT_"+CBin_strs[ibin]+"_"+CBin_strs[ibin+1]+"_Pt100_Pt300"));
  ...
  for(int fi = 0; fi < (int) file_names.size(); fi++) {
	...
	*/   
	for(int j4i = 0; j4i < (int) my_primary->jtpt->size(); j4i++) {
	  ...
	  if(( my_primary->jtpt->at(j4i) > pTmincut )&&(my_primary->trackMax->at(j4i)/my_primary->jtpt->at(j4i) > 0.01)){
	  ...
	  if(is_inclusive == kTRUE &&j4i!=highest_idx){
	    my_hists[data_mc_type_code]->only_nonleadingjets_corrpT[ibin][ibin2]->Fill(my_primary->jtpt->at(j4i), wvz*wcen); 
	  ...
	  if(j4i==highest_idx){
	    my_hists[data_mc_type_code]->only_leadingjets_corrpT[ibin][ibin2]->Fill(my_primary->jtpt->at(j4i), wvz*wcen);
	  ...
	  if(j4i==second_highest_idx){
	    my_hists[data_mc_type_code]->only_subleadingjets_corrpT[ibin][ibin2]->Fill(my_primary->jtpt->at(j4i), wvz*wcen);
	...
	*/
	...
	for(int j4i = 0; j4i < (int) my_primary->genpt->size(); j4i++) {
	  ...
	  cout<<"here starting track loop"<<endl;
	  for(int tracks =0; tracks < (int) my_primary->trkPt->size(); tracks++){
	    if(fabs(my_primary->trkEta->at(tracks))>=trketamaxcut) continue;
	    if (my_primary->highPurity->at(tracks)!=1) continue;
	    if(my_primary->trkPt->at(tracks)<=trkPtCut) continue;
	  ...
	  for(int j4i_reco = 0; j4i_reco < (int) my_primary->jtpt->size(); j4i_reco++) {
	  ...
	  if((closest_dr < 0.3)&&(my_primary->jtpt->at(closest_j4i)>120.)){
	    data_mc_type_code = 5;
	  }else if((closest_dr < 0.3)&&(my_primary->jtpt->at(closest_j4i)<=120.)){
	      data_mc_type_code = 6;
	  }else{
	    data_mc_type_code = 7;
	  }
	...
	while(mevi< meptrig && startovercheck < 2){  // If we've started over once already, going through a 2nd time won't do any good.
	  ...
	  for(int j4i = 0; j4i < (int) my_primary->jtpt->size(); j4i++){
	    ...
	    if( fabs(my_primary->jteta->at(j4i)) > etacut ) continue;
	    if( my_primary->jtpt->at(j4i) > pTmaxcut ) continue;
	    if( my_primary->jtpt->at(j4i) > pTmincut){ is_inclusive = kTRUE; } 
