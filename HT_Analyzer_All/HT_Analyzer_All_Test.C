...
int main(int argc, char *argv[]){
  ...
  double cent, eta, pt, phi, rmin, r_reco, jeteta, jetphi, fake, eff, secondary, multrec, trkweight,vz,deta, dphi, reco_eta, gen_eta, reco_phi, gen_phi, dr, closest_dr;
  ...
  //---------- Then we find gen dijets-----------------
	...
	if(closest_dr > 0.3){continue;}
	if((j4i==highest_idx)&&(my_primary->genpt->at(closest_j4i)<120.)){continue;}
	if(is_inclusive && (my_primary->genpt->at(closest_j4i)<120.)){ is_inclusive = kFALSE;}
	  ...
	  */
	  if(!is_data){
	    ...
	    for(int tracks =0; tracks < (int) my_primary->pt->size(); tracks++){
	      ...
	      if(my_primary->sube->at(tracks)!=0) continue;  //only hydro for these closures
